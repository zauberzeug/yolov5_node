...
def _calc_batch_size(queue: Queue, training_path: str, model_file: str, hyp_path: str, dataset_path: str, img_size: int) -> None:
    ...
    for batch_size in [128,64,32,16,8,4,2,1]:
        ...
        logging.info(f'estimated_total_size for batch_size {batch_size} and image_size {img_size} is {estimated_total_size}')
        ...
        if estimated_total_size < free_mem:
            ...
            logging.info(f'batch_size {batch_size} and image_size {img_size} is too big to fit in available memory {free_mem} ')
