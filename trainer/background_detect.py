...
class ContinousDetector:
    ...
    async def get_projects(self):
        async with loop.get('api/projects') as response:
            ...
            self.projects = [Project.from_dict(project)
                             for project in (await response.json())['projects']]
    ...
    async def get_model_id_from_deployment_target(self, organization: str, project: str, deployment_target: str) -> str:
        async with loop.get(f'/api/{organization}/projects/{project}/models') as response:
            ...
            target_model_id = [
                model['id'] for model in models if model['version'] == deployment_target]
            if target_model_id:
...
async def main():
    ...
    for project in cd.projects:
        if project.project_id == 'roboter':
            context = Context(
                organization=project.organization_id, project=project.project_id)
            deployment_target = await cd.get_deployment_target(project.project_id, project.organization_id)
            if deployment_target is not None:
                ids = await cd.get_image_ids_without_detections(
                    project.project_id, project.organization_id, deployment_target)
                ...
                await Trainer('yolov5_pytorch')._upload_detections(
                    context, jsonable_encoder(detections))
